...
struct Wheel
{
	...
	vec3 direction; 
	vec3 axis;
	float suspensionRestLength; // max length for suspension in meters, Suspenció monster truck
	float radius;
	float width;
	bool front; // is front wheel ?
	bool drive; // does this wheel received engine power ? quines rodes paren
...
public:
	...
	void Render();
	void ApplyEngineForce(float force); //Perque giri
